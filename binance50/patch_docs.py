with open("docs/ARCHITECTURE.md", "a") as f:
    ...
    f.write("- **Stream Parser**: Deserializes raw string payloads into strongly-typed Pydantic event models.\n")
    f.write("- **Stream Buffer**: Implements max capacity and drop policies (e.g., `reject_new`) for event queues.\n")
    f.write("- **Stream Simulator/Replay**: Loads offline JSON fixtures, executes pipeline steps sequentially, validating parsing and buffer handling without actual network.\n")
...
with open("docs/SECURITY.md", "a") as f:
    ...
    f.write("- Stream connections are strictly blocked by `stream_guard` if `market_stream_real_connect_enabled` is false.\n")
...
with open("README.md", "a") as f:
    f.write("\n## WebSocket Market Stream Layer\n")
    f.write("Phase 9 provides robust offline simulation and structural preparation for real-time WebSocket market data. Real websocket connections are intentionally disabled by default.\n")
