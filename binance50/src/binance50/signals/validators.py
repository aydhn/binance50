...
def validate_scored_candidate(scored: ScoredSignalCandidate, config: AppConfig) -> None:
    ...
    if config.signals.execution_forbidden or config.signals.quality.reject_execution_intent:
        if (
            "execution" in scored.intent.value.lower()
            or "order" in scored.intent.value.lower()
            and "no" not in scored.intent.value.lower()
        ):
            from binance50.core.exceptions import SignalValidationError

            raise SignalValidationError(
                f"Execution intent detected for candidate {getattr(scored, 'scored_signal_id', 'unknown')}"
            )

    if config.signals.quality.reject_score_out_of_range:
        if (
            scored.score < config.signals.scoring.min_score
            or scored.score > config.signals.scoring.max_score
        ):
            from binance50.core.exceptions import ScoreOutOfRangeError

            raise ScoreOutOfRangeError(
                f"Score {scored.score} out of bounds for candidate {getattr(scored, 'scored_signal_id', 'unknown')}"
            )
