...
class SignalScoringEngine:
    def __init__(
        self,
        config: AppConfig,
        weighting_engine: SignalWeightingEngine | None = None,
    ...
    def build_score_breakdown(
        self, candidate: SignalCandidate, components: list[SignalScoreComponent]
    ) -> SignalScoreBreakdown:
        subtotal = 0.0
        total_penalty = 0.0
    ...
    def compute_final_score(
        self, components: list[SignalScoreComponent], config: AppConfig
    ) -> float:
        ...
        return clamp_score(
            total, config.signals.scoring.min_score, config.signals.scoring.max_score
        )

    def score_candidate(
        self,
        candidate: SignalCandidate,
        group: ConfluenceGroup | None,
        ...
        if candidate.status in (StrategyCandidateStatus.rejected, StrategyCandidateStatus.warning):
            return self.reject_candidate(
                candidate, f"Candidate was previously {candidate.status.value}"
            )
        ...
        open_ts = (
            ...
            if isinstance(candidate.open_time, datetime.datetime)
        ...
        confluence_comp = (
            self.confluence_engine.compute_confluence_score(group, self.config)
            if group
            else self.weighting_engine.compute_component_contribution(
                "confluence", 0.0, self.config, "No confluence group"
            )
        )

        conf_comp_bonus = (
            self.confluence_engine.compute_confirmation_score(candidate, group, self.config)
            if group
            else self.weighting_engine.compute_component_contribution(
        ...
        freshness_comp = compute_freshness_component(
            candidate, curr_time, candidate.interval, self.config
        )
    ...
    def score_candidates(
        self, candidates: list[SignalCandidate], current_open_time: int | None = None
    ) -> list[ScoredSignalCandidate]:
        from binance50.signals.conflicts import (
            detect_opposite_direction_conflicts,
            detect_same_plugin_conflicts,
        )
        ...
        if current_open_time is None and candidates:
            ...
            for c in candidates:
                ts = (
                    ...
                    if isinstance(c.open_time, datetime.datetime)
                    else c.open_time
                )
        ...
        scored_results = []
        for c in candidates:
            c_group = candidate_groups.get(c.candidate_id)
