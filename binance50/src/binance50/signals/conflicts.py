...
def group_candidates_by_symbol_interval_time(
    candidates: list[SignalCandidate],
) -> dict[str, list[SignalCandidate]]:
    groups: dict[str, list[SignalCandidate]] = {}
    for c in candidates:
        time_key = (
            c.open_time.timestamp() * 1000 if hasattr(c.open_time, "timestamp") else c.open_time
        )
...
def detect_opposite_direction_conflicts(
    ...
    if (
        not config.signals.conflicts.enabled
        or not config.signals.conflicts.detect_opposite_direction_same_bar
    ):
        return []
    ...
    for key, group in groups.items():
        ...
        if bullish and bearish:
            ...
            time_key = (
                bullish[0].open_time.timestamp() * 1000
                if hasattr(bullish[0].open_time, "timestamp")
                else bullish[0].open_time
            )
            ...
            conflict = SignalConflict(
                ...
                reasons=[
                    f"Opposite direction signals detected on same bar ({len(bullish)} bullish, {len(bearish)} bearish)"
                ],
            )
...
def detect_same_plugin_conflicts(
    candidates: list[SignalCandidate], config: AppConfig
) -> list[SignalConflict]:
    if (
        not config.signals.conflicts.enabled
        or not config.signals.conflicts.detect_same_plugin_opposite_direction
    ):
        return []
    ...
    for key, group in groups.items():
        ...
        if conflicting_plugins:
            bullish = [
                c
                for c in group
                if c.plugin_name in conflicting_plugins and c.direction == StrategyDirection.bullish
            ]
            bearish = [
                c
                for c in group
                if c.plugin_name in conflicting_plugins and c.direction == StrategyDirection.bearish
            ]

            time_key = (
                group[0].open_time.timestamp() * 1000
                if hasattr(group[0].open_time, "timestamp")
                else group[0].open_time
            )
            ...
            conflict = SignalConflict(
                ...
                reasons=[
                    f"Plugin(s) {', '.join(conflicting_plugins)} generated opposite signals on the same bar"
                ],
            )
            conflicts.append(conflict)
...
def compute_conflict_penalty(
    candidate: SignalCandidate, conflicts: list[SignalConflict], config: AppConfig
) -> SignalScoreComponent:
    if not config.signals.conflicts.enabled or candidate.direction not in (
        ...
        return SignalScoreComponent(
            ...
            weight=0.0,
            contribution=0.0,
            reason="No conflict penalty applicable",
        )
    ...
    for conflict in conflicts:
        if (
            candidate.candidate_id in conflict.bullish_candidate_ids
            or candidate.candidate_id in conflict.bearish_candidate_ids
        ):
...
def mark_conflicted_scored_candidates(
    scored: list[ScoredSignalCandidate], conflicts: list[SignalConflict]
