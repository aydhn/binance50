...
class ConfluenceEngine:
    ...
    def assign_candidates_to_groups(
        ...
        for c in candidates:
            if c.direction in (StrategyDirection.bullish, StrategyDirection.bearish):
                time_key = (
                    c.open_time.timestamp() * 1000
                    if hasattr(c.open_time, "timestamp")
                    else c.open_time
                )
    ...
    def detect_plugin_type_diversity(
        self, group_candidates: list[SignalCandidate]
    ) -> dict[str, int]:
    ...
    def detect_confirmation_relationships(
        self, group_candidates: list[SignalCandidate]
    ) -> dict[str, bool]:
    ...
    def build_confluence_groups(
        self, candidates: list[SignalCandidate], config: AppConfig
        ...
        for key, group_candidates in directional_groups.items():
            ...
            time_key = (
                c0.open_time.timestamp() * 1000
                if hasattr(c0.open_time, "timestamp")
                else c0.open_time
            )
            ...
            opposite_direction = (
                ...
                if c0.direction == StrategyDirection.bullish
                else StrategyDirection.bullish
            )
            ...
            if len(plugin_names) >= config.signals.confluence.min_plugins_for_confluence:
                ...
                if (
                    config.signals.confluence.require_distinct_plugin_types
                    and len(plugin_types) > 1
                ):
                    confluence_score += config.signals.confluence.plugin_type_diversity_bonus

            confluence_score = clamp_score(
                confluence_score, 0.0, config.signals.confluence.max_confluence_score
            )
    ...
    def compute_confluence_score(
        self, group: ConfluenceGroup, config: AppConfig
    ) -> SignalScoreComponent:
        if (
            not config.signals.confluence.enabled
            or not group
            or group.same_direction_count < config.signals.confluence.min_plugins_for_confluence
        ):
            return SignalScoreComponent(
                ...
                weight=0.0,
                contribution=0.0,
                reason="Confluence conditions not met",
            )
        ...
        return SignalScoreComponent(
            ...
            metadata={
                "same_direction_count": group.same_direction_count,
                "plugin_types": group.plugin_types,
            },
        )

    def compute_confirmation_score(
        self, candidate: SignalCandidate, group: ConfluenceGroup, config: AppConfig
    ) -> SignalScoreComponent:
        if not config.signals.confluence.enabled or not group:
            return SignalScoreComponent(
                ...
                weight=0.0,
                contribution=0.0,
                reason="No group for confirmation",
            )
        ...
        if (
            relationships.get("divergence")
            and candidate.plugin_type.value != "divergence_candidate"
        ):
        ...
        return SignalScoreComponent(
            ...
            contribution=contribution,
            reason=(
                "Confirmed by: " + ", ".join(reasons) if reasons else "No specific confirmations"
            ),
