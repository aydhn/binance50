...
def detect_score_out_of_range(
    scored: list[ScoredSignalCandidate], config: AppConfig
) -> list[SignalQualityIssue]:
        ...
        if s.score < min_score or s.score > max_score:
            issues.append(
                SignalQualityIssue(
                    ...
                    severity=(
                        "error" if config.signals.quality.reject_score_out_of_range else "warning"
                    ),
        ...
        if not s.score_breakdown:
            issues.append(
                SignalQualityIssue(
                    ...
                    severity=(
                        "error" if config.signals.quality.reject_missing_breakdown else "warning"
                    ),
                    scored_signal_id=s.scored_signal_id,
        ...
        if not s.explanation:
            issues.append(
                SignalQualityIssue(
                    ...
                    severity=(
                        "error" if config.signals.quality.reject_missing_explanation else "warning"
                    ),
                    scored_signal_id=s.scored_signal_id,
        ...
        if "execution" in intent_val or "order" in intent_val and "no" not in intent_val:
            issues.append(
                SignalQualityIssue(
                    ...
                    severity=(
                        "error" if config.signals.quality.reject_execution_intent else "warning"
                    ),
                    scored_signal_id=s.scored_signal_id,
...
def detect_high_conflict_ratio(
    scored: list[ScoredSignalCandidate], config: AppConfig
) -> list[SignalQualityIssue]:
    if not scored:
        return []
    ...
    if (
        config.signals.quality.warn_high_conflict_ratio
        and ratio > config.signals.quality.max_conflict_ratio
    ):
        return [
            SignalQualityIssue(
...
def detect_single_plugin_high_scores(
    scored: list[ScoredSignalCandidate], config: AppConfig
) -> list[SignalQualityIssue]:
    issues: list[SignalQualityIssue] = []
...
def build_signal_quality_report(
    result_or_scored: SignalScoringResult | list[ScoredSignalCandidate], config: AppConfig
) -> SignalQualityReport:
    scored_candidates = (
        result_or_scored.scored_candidates
        if isinstance(result_or_scored, SignalScoringResult)
        else result_or_scored
    )
    ...
    return SignalQualityReport(
        ...
        rejected_count=(
            len(result_or_scored.rejected_candidates)
            if isinstance(result_or_scored, SignalScoringResult)
            else 0
        ),
