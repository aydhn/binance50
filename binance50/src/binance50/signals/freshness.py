...
def compute_candidate_age_bars(candidate: SignalCandidate, current_open_time: int, interval: str) -> int:
    cand_time = candidate.open_time.timestamp() * 1000 if isinstance(candidate.open_time, datetime) else candidate.open_time
...
def is_candidate_expired(candidate: SignalCandidate, current_open_time: int, interval: str, config: AppConfig) -> bool:
    if not config.signals.freshness.enabled:
        return False

    age = compute_candidate_age_bars(candidate, current_open_time, interval)
    expiry = min(candidate.expiry_bars or config.signals.freshness.default_expiry_bars, config.signals.freshness.max_expiry_bars)

    return age > expiry


def compute_freshness_multiplier(candidate: SignalCandidate, current_open_time: int, interval: str, config: AppConfig) -> float:
    if not config.signals.freshness.enabled:
        return 1.0
    ...
    if config.signals.freshness.score_decay_mode == "linear":
        decay_range = config.signals.freshness.current_bar_multiplier - config.signals.freshness.stale_score_multiplier
        decay_per_bar = decay_range / max(expiry, 1)
        return max(config.signals.freshness.stale_score_multiplier, config.signals.freshness.current_bar_multiplier - (decay_per_bar * age))
    else:
        return config.signals.freshness.stale_score_multiplier


def compute_freshness_component(candidate: SignalCandidate, current_open_time: int, interval: str, config: AppConfig) -> SignalScoreComponent:
    if not config.signals.freshness.enabled:
        return SignalScoreComponent(
            name="freshness", raw_value=100.0, normalized_value=100.0, weight=0.0, contribution=0.0, reason="Freshness disabled"
        )
