...
def sanitize_path_component(value: str) -> str:
    ...
    if any(
        secret_word in lower_val
        for secret_word in ["api_key", "secret", "token", "signature", "password"]
    ):
...
def build_dataset_partition_path(
    ...
    parts.append(
        f"dataset={ds_name}" if config.storage.parquet.partition_style == "hive" else ds_name
    )

    if market_scope and config.storage.partitioning.by_market_scope:
        parts.append(
            f"market_scope={sanitize_path_component(market_scope)}"
            if config.storage.parquet.partition_style == "hive"
            else sanitize_path_component(market_scope)
        )

    if symbol and config.storage.partitioning.by_symbol:
        parts.append(
            f"symbol={sanitize_path_component(symbol)}"
            if config.storage.parquet.partition_style == "hive"
            else sanitize_path_component(symbol)
        )

    if interval and config.storage.partitioning.by_interval:
        parts.append(
            f"interval={sanitize_path_component(interval)}"
            if config.storage.parquet.partition_style == "hive"
            else sanitize_path_component(interval)
        )

    if year and config.storage.partitioning.by_year:
        parts.append(
            f"year={sanitize_path_component(str(year))}"
            if config.storage.parquet.partition_style == "hive"
            else sanitize_path_component(str(year))
        )

    if month and config.storage.partitioning.by_month:
        parts.append(
            f"month={sanitize_path_component(str(month).zfill(2))}"
            if config.storage.parquet.partition_style == "hive"
            else sanitize_path_component(str(month).zfill(2))
        )

    if day and config.storage.partitioning.by_day:
        parts.append(
            f"day={sanitize_path_component(str(day).zfill(2))}"
            if config.storage.parquet.partition_style == "hive"
            else sanitize_path_component(str(day).zfill(2))
        )
