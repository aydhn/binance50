...
class DataIndex:
    ...
    def build_coverage_from_ohlcv(
        self, df: pd.DataFrame, version_id: str, quality_status: str = "pass"
    ...
    def upsert_coverage(self, records: list[DataCoverageRecord]) -> None:
        ...
        with self.catalog.transaction() as c:
            for r in records:
                # Check if exists
                c.execute(
                    "SELECT coverage_id FROM data_index WHERE dataset_name=? AND market_scope=? AND symbol=? AND interval=?",
    ...
    def get_coverage(
        self, symbol: str, interval: str, market_scope: str
    ) -> DataCoverageRecord | None:
