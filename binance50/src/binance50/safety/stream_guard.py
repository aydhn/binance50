...
def assert_stream_config_safe(config: AppConfig) -> None:
    ...
    if config.streams.market_stream_real_connect_enabled:
        raise StreamConfigError("Real stream connection enabled in config, but not allowed in Phase 9")

    if config.streams.buffer_max_events < 100 or config.streams.buffer_max_events > 1_000_000:
        raise StreamConfigError(f"buffer_max_events {config.streams.buffer_max_events} is out of safe range")
...
def assert_subscription_plan_safe(plan: 'StreamSubscriptionPlan', config: AppConfig) -> None:
    ...
    if len(plan.subscriptions) > max_streams:
        raise StreamSubscriptionError(f"Plan stream count {len(plan.subscriptions)} exceeds safe limit {max_streams}")
    if plan.control_message_count > config.streams.max_control_messages_per_second_spot and plan.market_scope == MarketScope.SPOT:
    ...
    for sub in plan.subscriptions:
        if sub.route.value in ("usdm_private",):
            raise StreamSubscriptionError(f"Private routes not allowed in this phase: {sub.route.value}")
