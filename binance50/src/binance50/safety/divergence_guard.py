...
from binance50.core.exceptions import DivergenceDetectionError, RepaintingRiskError
...
def assert_divergence_candidates_safe(
    candidates: list[DivergenceSignalCandidate], config: AppConfig
) -> None:
    ...
    for cand in candidates:
        if cand.score < 0 or cand.score > max_score:
            raise DivergenceDetectionError(
                f"Candidate score {cand.score} out of bounds [0, {max_score}]"
            )
