...
def build_subscription_plan(
    ...
    if len(symbols) > config.streams.max_symbols_per_stream_plan:
        raise ValueError(
            f"Symbols count exceeds max_symbols_per_stream_plan ({config.streams.max_symbols_per_stream_plan})"
        )
...
def split_subscriptions_into_connections(
    plan: StreamSubscriptionPlan, config: AppConfig
) -> list[list[StreamSubscription]]:
    max_streams = (
...
def build_set_property_payload(combined: bool, request_id: int = 1) -> dict:
    return {"method": "SET_PROPERTY", "params": ["combined", combined], "id": request_id}
...
def validate_subscription_plan(plan: StreamSubscriptionPlan, config: AppConfig) -> None:
    ...
    if len(plan.subscriptions) > max_limit:
        raise StreamSubscriptionError(
            f"Subscription count {len(plan.subscriptions)} exceeds max limit {max_limit}"
        )
