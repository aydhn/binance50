...
def simulate_backtest_open_fill(risk_assessment: dict, next_bar: dict, config) -> BacktestFill:
    ...
    price = choose_backtest_fill_price(next_bar, config.backtest.timing.fill_model, side, True, config)
...
def simulate_backtest_close_fill(position, next_bar: dict, reason: str, config) -> BacktestFill:
    ...
    price = choose_backtest_fill_price(next_bar, config.backtest.timing.fill_model, close_side, False, config)
...
def choose_backtest_fill_price(bar: dict, fill_model: str, side: str, is_open: bool, config) -> float:
    return bar.get("open", 0.0)

def apply_backtest_slippage(price: float, side: str, is_open: bool, config, context=None) -> tuple[float, float]:
