...
def build_feature_metadata_for_columns(df: pd.DataFrame, config: AppConfig) -> List[FeatureMetadata]:
...
def build_feature_set_metadata(df: pd.DataFrame, config: AppConfig, input_hashes: Dict[str, str], config_hash: str, output_hash: str) -> FeatureSetMetadata:
...
def validate_feature_metadata(metadata: FeatureMetadata) -> None:
    ...
    if any(k in col_lower for k in TARGET_KEYWORDS):
        raise FeatureMetadataError(f"Target/label/future keyword found in feature name: {metadata.feature_name}")
