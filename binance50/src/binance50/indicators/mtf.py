...
def compute_mtf_tolerance_ms(base_interval: str, higher_interval: str, max_alignment_tolerance_bars: int) -> int:
...
def prepare_higher_tf_features(higher_df: pd.DataFrame, higher_interval: str, config: AppConfig) -> pd.DataFrame:
    df = higher_df.copy()
...
def align_higher_tf_to_base(base_df: pd.DataFrame, higher_df: pd.DataFrame, request: MTFAlignmentRequest, config: AppConfig) -> MTFAlignmentResult:
    ...
    aligned = pd.merge_asof(
        ...
        right_on='close_time',
        direction='backward',
        tolerance=pd.Timedelta(milliseconds=request.tolerance_ms) if request.tolerance_ms > 0 else None,
        suffixes=('', '_higher')
    )

    matched = aligned['close_time_higher'].notna().sum() if 'close_time_higher' in aligned.columns else aligned['close_time'].notna().sum()
    unmatched = len(aligned) - matched

    if cfg.drop_unmatched_rows:
        if 'close_time_higher' in aligned.columns:
            aligned = aligned.dropna(subset=['close_time_higher' if 'close_time_higher' in aligned.columns else 'close_time'])
    elif cfg.mark_unmatched_rows:
        aligned['mtf_unmatched'] = aligned['close_time_higher'].isna() if 'close_time_higher' in aligned.columns else aligned['close_time'].isna()
...
def validate_mtf_no_future_alignment(base_df: pd.DataFrame, aligned_df: pd.DataFrame, higher_time_column: str) -> None:
    ...
    if len(future_leaks) > 0:
        raise MTFLookaheadError(f"Found {len(future_leaks)} rows where higher TF data is from the future")
