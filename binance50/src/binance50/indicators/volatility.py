...
def donchian_channels(high: pd.Series, low: pd.Series, period: int = 20) -> pd.DataFrame:
    ...
    return pd.DataFrame({f"vol_donchian_high_{period}": upper, f"vol_donchian_low_{period}": lower})
...
def realized_volatility(close: pd.Series, period: int = 20) -> pd.Series:
    ...
    res = lr.rolling(window=period, min_periods=period).std() * np.sqrt(
        365 * 24 * 60
    )  # Assuming 1m data annualized? Actually, let's keep it simple standard dev of returns
