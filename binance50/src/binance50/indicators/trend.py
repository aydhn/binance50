...
def aroon(high: pd.Series, low: pd.Series, period: int = 14) -> pd.DataFrame:
    aroon_up = high.rolling(window=period + 1, min_periods=period + 1).apply(lambda x: 100 * (period - (period - x.argmax())) / period, raw=True)
    aroon_down = low.rolling(window=period + 1, min_periods=period + 1).apply(lambda x: 100 * (period - (period - x.argmin())) / period, raw=True)
