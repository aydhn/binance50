...
def build_indicator_v2_summary(result: IndicatorV2RunResult) -> dict[str, Any]:
    ...
    return {
        ...
        "request_id": result.request.request_id if result.request else None,
        "feature_set_id": (
            result.feature_set_metadata.feature_set_id if result.feature_set_metadata else None
        ),
        "feature_count": (
            result.feature_set_metadata.feature_count if result.feature_set_metadata else 0
        ),
...
def build_indicator_v2_health_report(
    config: AppConfig, feature_registry=None, pattern_registry=None
) -> dict[str, Any]:
