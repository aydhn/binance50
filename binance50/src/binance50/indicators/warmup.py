...
def compute_valid_row_mask(df: pd.DataFrame, indicator_columns: list[str], min_valid_ratio: float) -> pd.Series:
...
def summarize_warmup(df: pd.DataFrame, indicator_columns: list[str], max_lookback: int) -> dict[str, Any]:
...
def assert_sufficient_history(df: pd.DataFrame, specs: list[IndicatorSpec], config: AppConfig) -> None:
    min_required = config.indicators.min_rows_required
    max_lookback = estimate_max_lookback(specs)

    if len(df) < min_required:
        raise InsufficientHistoryError(f"Insufficient history: {len(df)} rows, minimum required is {min_required}")

    if len(df) <= max_lookback:
        raise InsufficientHistoryError(f"Insufficient history: {len(df)} rows, max lookback is {max_lookback}")
