...
def detect_causal_pivot_highs(series: pd.Series, left_window: int, min_prominence_pct: float, min_distance_bars: int) -> List[PivotPoint]:
            ...
            if prominence >= min_prominence_pct:
                if candidate_idx - last_pivot_idx >= min_distance_bars:
                    pivots.append(PivotPoint(
                        ...
                        open_time=indices[candidate_idx] if isinstance(indices, pd.DatetimeIndex) else pd.Timestamp.utcnow(),
                        index=candidate_idx,
...
def detect_causal_pivot_lows(series: pd.Series, left_window: int, min_prominence_pct: float, min_distance_bars: int) -> List[PivotPoint]:
...
def enforce_min_pivot_distance(pivots: List[PivotPoint], min_distance_bars: int) -> List[PivotPoint]:
    ...
    for current in pivots[1:]:
        ...
        if current.pivot_type == last.pivot_type:
            if current.index - last.index >= min_distance_bars:
                ...
                if current.pivot_type == PivotType.high and current.value > last.value:
                    result[-1] = current
                elif current.pivot_type == PivotType.low and current.value < last.value:
                    result[-1] = current
        else:
            result.append(current)
...
def detect_price_pivots(df: pd.DataFrame, source_column: str, config: AppConfig) -> List[PivotPoint]:
    ...
    highs = detect_causal_pivot_highs(series, cfg.left_window, cfg.min_prominence_pct, cfg.min_distance_bars)
    lows = detect_causal_pivot_lows(series, cfg.left_window, cfg.min_prominence_pct, cfg.min_distance_bars)
...
def detect_indicator_pivots(df: pd.DataFrame, indicator_column: str, config: AppConfig) -> List[PivotPoint]:
