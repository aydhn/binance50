...
from binance50.signals.models import ScoredSignalCandidate, SignalScoreTier, SignalDecisionIntent, ScoredSignalStatus, SignalScoreBreakdown, SignalScoreComponent
from binance50.safety.scoring_guard import assert_signal_config_safe, assert_no_execution_threshold, assert_scored_signals_safe, build_signal_scoring_safety_report
...
@pytest.fixture
def scored():
    bd = SignalScoreBreakdown(components=[], subtotal_before_penalties=50.0, total_penalty=0.0, final_score=50.0, score_tier=SignalScoreTier.medium)
    return ScoredSignalCandidate(
        ...
        score_breakdown=bd, explanation="Safe explanation"
    )
...
def test_assert_scored_signals_safe(scored, config):
    ...
    # Test intent failure
    from binance50.core.exceptions import SignalValidationError, ExecutionObjectDetectedError, ActionableLanguageDetectedError
