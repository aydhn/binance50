...
def test_vwap(sample_data):
    res = vwap(sample_data["high"], sample_data["low"], sample_data["close"], sample_data["volume"], 20)
    assert len(res) == 100
    assert res.name == "volu_vwap_20"

def test_mfi(sample_data):
    res = mfi(sample_data["high"], sample_data["low"], sample_data["close"], sample_data["volume"], 14)
    assert len(res) == 100
...
def test_cmf(sample_data):
    res = cmf(sample_data["high"], sample_data["low"], sample_data["close"], sample_data["volume"], 20)
    assert len(res) == 100
...
def test_adl(sample_data):
    res = accumulation_distribution_line(sample_data["high"], sample_data["low"], sample_data["close"], sample_data["volume"])
    assert len(res) == 100
