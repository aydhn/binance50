...
def test_build_score_breakdown(engine, candidate):
    components = [
        SignalScoreComponent(name="c1", raw_value=50.0, normalized_value=50.0, weight=1.0, contribution=50.0, reason="r1"),
        SignalScoreComponent(name="c2", raw_value=10.0, normalized_value=10.0, weight=-1.0, contribution=-10.0, reason="r2")
    ]
...
def test_compute_final_score(engine, config):
    components = [
        SignalScoreComponent(name="c1", raw_value=60.0, normalized_value=60.0, weight=1.0, contribution=60.0, reason="r1"),
        SignalScoreComponent(name="c2", raw_value=50.0, normalized_value=50.0, weight=1.0, contribution=50.0, reason="r2")
    ]
...
def test_score_candidate_full(engine, candidate):
    ...
    scored = engine.score_candidate(candidate, group=None, conflicts=[], current_open_time=int(candidate.open_time.timestamp()*1000) + 1)
