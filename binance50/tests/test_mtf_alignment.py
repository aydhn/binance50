...
from binance50.indicators.mtf import align_higher_tf_to_base, MTFAlignmentRequest, prepare_higher_tf_features
...
def test_backward_asof_alignment(config: AppConfig):
    ...
    h_df = pd.DataFrame({
        "close_time": [pd.Timestamp("2023-01-01 10:00:00"), pd.Timestamp("2023-01-01 10:05:00")],
        "is_closed": [True, True],
        "h_val": [100, 200]
    })

    req = MTFAlignmentRequest(
        symbol="BTCUSDT", market_scope="spot", base_interval="1m", higher_interval="5m",
        base_df_hash="x", higher_df_hash="y", tolerance_ms=5*60*1000,
        require_higher_tf_closed=True, alignment_method="asof_backward"
    )
...
def test_future_alignment_blocked(config: AppConfig):
    from binance50.config.models import MTFConfig
    ...
    req = MTFAlignmentRequest(
        symbol="BTCUSDT", market_scope="spot", base_interval="1m", higher_interval="5m",
        base_df_hash="x", higher_df_hash="y", tolerance_ms=5*60*1000,
        require_higher_tf_closed=True, alignment_method="forward"
    )

    base_df = pd.DataFrame({"open_time": pd.date_range("2023-01-01 10:00:00", periods=1, freq="min")})
