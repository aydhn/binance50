...
def test_stream_plan():
    result = runner.invoke(app, ["stream-plan", "--symbols", "BTCUSDT", "--scope", "spot", "--types", "kline", "--interval", "1m"])
    assert result.exit_code == 0
    assert "Subscription Plan" in result.stdout

def test_stream_url_test():
    result = runner.invoke(app, ["stream-url-test", "--symbols", "BTCUSDT", "--scope", "spot", "--types", "kline", "--interval", "1m"])
    assert result.exit_code == 0
