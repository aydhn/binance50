...
def test_compute_plugin_weighted_score(config, weighting_engine):
    cand = SignalCandidate(
        ...
        open_time=datetime.now(), plugin_name="test_plugin", plugin_type=StrategyPluginType.trend_following,
        direction=StrategyDirection.bullish, strength=StrategyCandidateStrength.medium,
        confidence=80.0, explanation={"summary": "test"} # mock dict, it gets validated usually but bypassing strict validation here
    )
