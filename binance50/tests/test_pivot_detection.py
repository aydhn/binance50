...
from binance50.indicators.pivots import (
    PivotType,
    detect_causal_pivot_highs,
    detect_causal_pivot_lows,
    detect_price_pivots,
)
...
def test_causal_pivot_high(config: AppConfig):
    ...
    pivots = detect_causal_pivot_highs(
        series, left_window=2, min_prominence_pct=0.01, min_distance_bars=2
    )
    assert len(pivots) == 2
...
def test_causal_pivot_low(config: AppConfig):
    ...
    pivots = detect_causal_pivot_lows(
        series, left_window=2, min_prominence_pct=0.01, min_distance_bars=2
    )
    assert len(pivots) == 2
    assert pivots[0].index == 2
